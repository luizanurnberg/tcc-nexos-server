...
from route.client.route import client_route
...
def create_app(config=None):
    ...
    try:
        ...
        print(
            f"Conectado ao banco de dados: {app.config.get('DATABASE_NAME', DATABASE_NAME)}"
        )
    ...
    try:
        cred = credentials.Certificate(
            app.config.get("FIREBASE_CREDENTIALS", FIREBASE_CREDENTIALS)
        )
